...
import { getTeamMemberRoleSuccess, getTeamMembersSuccess, hasError, inviteMemberSuccess, removeTeamMemberSuccess } from 'store/slices/team';
...
export const updateTeamMemberRoleService = (data) => {
  return async () => {
    try {
      const response = await axios.put(`teams/team`, data);
      dispatch(getTeamMemberRoleSuccess({ team: response.data.data }));
    } catch (error) {
      openErrorSnackbar(error.msg, 'error');
      dispatch(hasError());
    }
